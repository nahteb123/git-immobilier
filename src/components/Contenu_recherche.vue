  ...
  <div class="contenu_recherche">
    ...
    <div class="conteneur_resultat_recherche">
      <div v-for="appartement in appartements" :key="appartement.id_appartement" class="resultat_recherche">
        ...
        <img :src="appartement.chemin_photo" alt="Photo d'appartement" class="recherche_photo" />
        <div class="recherche_info_appart">
        <div class="info_appart">
          <h1> N° {{ appartement.id_appartement }}</h1>
          <h2>Lieu: {{ appartement.nom_ville }}</h2>
          <h2>Adresse: {{ appartement.adresse }}</h2>
        </div>
        
        <h2 class="prix_recherche">Prix: {{ appartement.loyer }}€</h2>
      </div>
        <button class="bouton_lien_crea_appart" @click="voirDetails(appartement.id_appartement)">Voir les détails de cet appartement</button>
      </div>
    </div>
